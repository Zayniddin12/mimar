export const offers = [
  {
    id: 1,
    title: 'navbar.expert_team',
    description: 'navbar.expert_team_subtittle',
  },
  {
    id: 2,
    title: 'navbar.quality',
    description: 'navbar.quality_subtittle',
  },
  {
    id: 3,
    title: 'navbar.customizead',
    description: 'navbar.customized_subtittle',
  },
  {
    id: 4,
    title: 'navbar.end_to_project_title',
    description: 'navbar.end_to_project_subtittle',
  },
];
export const targets = [
  {
    id: 1,
    title: 'navbar.mission',
    description: 'navbar.mission_desc',
  },
  {
    id: 2,
    title: 'navbar.vision',
    description: 'navbar.vission_desc',
  },
  {
    id: 3,
    title: 'navbar.target',
    description: 'navbar.target_desc',
  },

];
export const values = [
  {
    id: 1,
    title: 'navbar.excellence',
    description: 'navbar.excellence_desc',
    image: '/images/enginering.png',
  },
  {
    id: 2,
    title: 'navbar.safety',
    description: 'navbar.safety_desc',
    image: '/images/safety.png',
  },
  {
    id: 3,
    title: 'navbar.innovation',
    description: 'navbar.innovation_desc',
    image: '/images/inovation.png',
  },

];
export const projects = [
  {
    images: ['/images/esg2.png', '/images/esg1.png', '/images/AboutUs.png'],
    title: 'Shaxidlar maydoni',
    area: '11 ha',
    timeline: '2023 (Q3)',
    description: 'A historic site renovation with a new amphitheater and administrative building, celebrating the country’s heritage.',
  },
  {
    images: ['/images/esg1.png', '/images/AboutUs.png', '/images/esg2.png'],
    title: 'Shaxidlar maydoni',
    area: '11 ha',
    timeline: '2023 (Q3)',
    description: 'A historic site renovation with a new amphitheater and administrative building, celebrating the country’s heritage.',
  },
];
export const engineers = [
  {
    id: 1,
    images: '/images/enginer1.png',
    title: 'navbar.Early',
    description: 'navbar.Early_subtittle',
    list: [
      {
        title: 'navbar.early_ul1',
        id: 1,
      },
      {
        title: 'navbar.early_ul2',
        id: 2,
      },
      {
        title: 'navbar.early_ul3',
        id: 3,
      },
      {
        title: 'navbar.early_ul4',
        id: 4,
      },
    ],
  },
  {
    images: '/images/enginer2.png',
    title: 'navbar.Front-End',
    description: 'navbar.Front-End_subtittle',
    list: [
      {
        title: 'navbar.front_ul1',
        id: 1,
      },
      {
        title: 'navbar.front_ul2',
        id: 2,
      },
      {
        title: 'navbar.front_ul3',
        id: 3,
      },
      {
        title: 'navbar.front_ul4',
        id: 4,
      },
      {
        title: 'navbar.front_ul5',
        id: 5,
      },
      {
        title: 'navbar.front_ul6',
        id: 6,
      },
      {
        title: 'navbar.front_ul7',
        id: 7,
      },
      {
        title: 'navbar.front_ul8',
        id: 8,
      },
    ],
  },
  {
    images: '/images/enginer3.png',
    title: 'navbar.Detailed',
    description: 'navbar.Detailed_subtittle',
    list: [
      {
        title: 'navbar.detailed_ul1',
        id: 1,
      },
      {
        title: 'navbar.detailed_ul2',
        id: 2,
      },
      {
        title: 'navbar.detailed_ul3',
        id: 3,
      },
      {
        title: 'navbar.detailed_ul4',
        id: 4,
      },
      {
        title: 'navbar.detailed_ul5',
        id: 5,
      },
    ],
  },
];
export const procurement = [
  {
    id: 1,
    images: '/images/procurement1.png',
    title: 'navbar.Supplier',
    description: 'navbar.Supplier_subtittle',
  },
  {
    images: '/images/procurement2.png',
    title: 'navbar.Cost',
    description: 'navbar.Cost_subtittle',
  },

];

export const constructions = [
  {
    id: 1,
    images: '/images/construction2.png',
    title: 'navbar.Project',
    description: 'navbar.Project_subtittle',
  },
  {
    images: '/images/construction3.png',
    title: 'navbar.Commissioning',
    description: 'navbar.Commissioning_subtittle',
  },

];

export const expertises = [
  {
    id: 1,
    title: 'navbar.Non-Ferrous',
    description: 'navbar.Non-Ferrous_subtittle',
    image: '/images/expertise1.png',
  },
  {
    id: 2,
    title: 'navbar.Construction',
    description: 'navbar.Construction_subtittle',
    image: '/images/expertise2.png',
  },
  {
    id: 3,
    title: 'navbar.Refineries',
    description: 'navbar.Refineries_subtittle',
    image: '/images/expertise3.png',
  },
  {
    id: 4,
    title: 'navbar.Cement',
    description: 'navbar.Cement_subtittle',
    image: '/images/expertise4.png',
  },
  {
    id: 5,
    title: 'navbar.Power',
    description: 'navbar.Power_subtittle',
    image: '/images/expertise5.png',
  },
  {
    id: 6,
    title: 'navbar.Process',
    description: 'navbar.Process_subtittle',
    image: '/images/expertise6.png',
  },

];
export const esg = [
  {
    id: 1,
    title: 'navbar.Energy',
    description: 'navbar.Energy_subtittle',
    image: '/images/esgp2.png',
  },
  {
    id: 2,
    title: 'navbar.Job',
    description: 'navbar.Job_subtittle',
    image: '/images/esgp3.png',
  },
  {
    id: 3,
    title: 'navbar.Solar',
    description: 'navbar.Solar_subtittle',
    image: '/images/esgp4.png',
  },
  {
    id: 4,
    title: 'navbar.Shahidlar',
    description: 'navbar.Shahidlar_subtittle',
    image: '/images/esgp5.png',
  },
  {
    id: 5,
    title: 'navbar.Pesticide',
    description: 'navbar.Pesticide_subtittle',
    image: '/images/esgp6.png',
  },
];
export const esgList = [
  {
    id: 1,
    title: 'navbar.Environmental',
    description: 'navbar.Environmental_subtittle',
  },
  {
    id: 2,
    title: 'navbar.Social',
    description: 'navbar.Social_subtittle',
  },
  {
    id: 3,
    title: 'navbar.Governance',
    description: 'Our commitment to ethical business practices, transparency, and compliance with all regulations ensures that we build trust with stakeholders and deliver on our promises.',
  },

];