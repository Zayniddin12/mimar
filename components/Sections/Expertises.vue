<template>
  <section>
    <div class="container">
      <h2 class="3xl:mb-[48px] 2xl:mb-[36px] lg:mb-[30px] mb-[24px] font-alumni">{{ $t('navbar.expertises') }}</h2>

      <div class="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 gap-2 font-golos">
        <NuxtLinkLocale
          to="/expertises"
          class="cursor-pointer p-6 min-h-[184px] mt-4 border border-[#E2E8F0] bg-white flex flex-col justify-between transition-300 hover:-translate-y-1 hover:shadow-md">
          <div class="flex justify-between">
            <img src="/icons/card-icon.png" class="lg:h-5 h-[18px]" alt="Icon">

            <img src="/icons/arrow-right.png" alt="Right icon">
          </div>
          <div>
            <p class="2xl:text-[24px] lg:text-[22px] text-[18px] leading-[130%] font-golos text-black">
              {{ $t('navbar.facility_refineries') }}</p>
          </div>
        </NuxtLinkLocale>

        <NuxtLinkLocale
          to="/expertises"
          class="cursor-pointer p-6 min-h-[184px] mt-4 border border-[#E2E8F0] bg-custom flex flex-col justify-between  transition-300 hover:-translate-y-1 hover:shadow-md">
          <div class="flex justify-between">
            <img src="/icons/card-icon2.png" class="lg:h-5 h-[18px]" alt="Icon">

            <img src="/icons/arrow-right2.png" alt="Right icon">
          </div>
          <div class="mt-1">
            <p class="2xl:text-[24px] lg:text-[22px] text-[18px] leading-[130%] font-golos text-white">
              {{ $t('navbar.power_plan') }}</p>
          </div>
        </NuxtLinkLocale>
        <NuxtLinkLocale
          to="/expertises"
          class="cursor-pointer p-6 min-h-[184px] mt-4 border border-[#E2E8F0] bg-white flex flex-col justify-between  transition-300 hover:-translate-y-1 hover:shadow-md">
          <div class="flex justify-between">
            <img src="/icons/card-icon.png" class="lg:h-5 h-[18px]" alt="Icon">

            <img src="/icons/arrow-right.png" alt="Right icon">
          </div>
          <div>
            <p class="2xl:text-[24px] lg:text-[22px] text-[18px] leading-[130%] font-golos text-black">
              {{ $t('navbar.mining_industury') }}</p>
          </div>
        </NuxtLinkLocale>

        <NuxtLinkLocale
          to="/expertises"
          class="cursor-pointer p-6 min-h-[184px] mt-4 border border-[#E2E8F0] bg-custom flex flex-col justify-between  transition-300 hover:-translate-y-1 hover:shadow-md">
          <div class="flex justify-between">
            <img src="/icons/card-icon2.png" class="lg:h-5 h-[18px]" alt="Icon">

            <img src="/icons/arrow-right2.png" alt="Right icon">
          </div>
          <div class="mt-1">
            <p class="2xl:text-[24px] lg:text-[22px] text-[18px] leading-[130%] font-golos text-white">
              {{ $t('navbar.metal_recovery') }}</p>
          </div>
        </NuxtLinkLocale>
        <NuxtLinkLocale
          to="/expertises"
          class="cursor-pointer p-6 min-h-[184px] mt-4 border border-[#E2E8F0] bg-white flex flex-col justify-between transition-300 hover:-translate-y-1 hover:shadow-md">
          <div class="flex justify-between">
            <img src="/icons/card-icon.png" class="lg:h-5 h-[18px]" alt="Icon">

            <img src="/icons/arrow-right.png" alt="Right icon">
          </div>
          <div>
            <p class="2xl:text-[24px] lg:text-[22px] text-[18px] leading-[130%] font-golos text-black">
              {{ $t('navbar.environmental_tecknologies') }}</p>
          </div>
        </NuxtLinkLocale>

        <NuxtLinkLocale
          to="/expertises"
          class="cursor-pointer p-6 min-h-[184px] mt-4 border border-[#E2E8F0] bg-custom flex flex-col justify-between  transition-300 hover:-translate-y-1 hover:shadow-md">
          <div class="flex justify-between">
            <img src="/icons/card-icon2.png" class="lg:h-5 h-[18px]" alt="Icon">

            <img src="/icons/arrow-right2.png" alt="Right icon">
          </div>
          <div class="mt-1">
            <p class="2xl:text-[24px] lg:text-[22px] text-[18px] leading-[130%] font-golos text-white">
              {{ $t('navbar.process_equipment') }}</p>
          </div>
        </NuxtLinkLocale>
        <NuxtLinkLocale
          to="/expertises"
          class="cursor-pointer p-6 min-h-[184px] mt-4 border border-[#E2E8F0] bg-white flex flex-col justify-between  transition-300 hover:-translate-y-1 hover:shadow-md">
          <div class="flex justify-between">
            <img src="/icons/card-icon.png" class="lg:h-5 h-[18px]" alt="Icon">

            <img src="/icons/arrow-right.png" alt="Right icon">
          </div>
          <div>
            <p class="2xl:text-[24px] lg:text-[22px] text-[18px] leading-[130%] font-golos text-black">
              {{ $t('navbar.tanks_pressure') }}</p>
          </div>
        </NuxtLinkLocale>

        <NuxtLinkLocale
          to="/expertises"
          class="cursor-pointer p-6 min-h-[184px] mt-4 border border-[#E2E8F0] bg-custom flex flex-col justify-between  transition-300 hover:-translate-y-1 hover:shadow-md">
          <div class="flex justify-between">
            <img src="/icons/card-icon2.png" class="lg:h-5 h-[18px]" alt="Icon">

            <img src="/icons/arrow-right2.png" alt="Right icon">
          </div>
          <div class="mt-1">
            <p class="2xl:text-[24px] lg:text-[22px] text-[18px] leading-[130%] font-golos text-white">
              {{ $t('navbar.construction_civil') }}</p>
          </div>
        </NuxtLinkLocale>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">

</script>
